...
const Reviews = () => {
  return (
    ...
    <h2 className='text-2xl font-semibold pl-4 md:pl-16 pb-16'>Reviews<span className='animate-pulse'>/ </span></h2>
    <Marquee speed={25}>
      {reviews.map((review,index)=>(
        <ReviewCard key={index} name={review.name} imageSrc={review.imgSrc} text={review.text} stars={review.stars}/>
      ))}
    </Marquee>
  </div>
  )
